...
public sealed partial class PromoteService
{
    ...
    private static void _PerformAsPromoteProcess(string pid)
    {
        ...
        // 验证来源
        var mainProcessPath = new FileInfo(process.MainModule!.FileName).FullName;
        var executablePath = new FileInfo(Basics.ExecutablePath).FullName;
        if (!string.Equals(mainProcessPath, executablePath, StringComparison.OrdinalIgnoreCase))
        {
