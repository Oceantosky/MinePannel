...
public class HttpProxyManager : IWebProxy, IDisposable
{
    ...
    public static event Action<string>? OnSystemProxyChangedNotify;

    private void _onSystemProxyChanged(object? sender, EventArgs e)
    {
        RefreshSystemProxy(true);
    }

    /// <summary>刷新系统代理设置</summary>
    public void RefreshSystemProxy(bool notify = false)
    {
        lock (_lock)
        {
            try
            {
                ...
                if (systemProxyAddress is not null && !systemProxyAddress.StartsWith("http")) 
                {
                    if (systemProxyAddress.EndsWith(":443"))
                        systemProxyAddress = $"https://{systemProxyAddress}/";
                    else
                        systemProxyAddress = $"http://{systemProxyAddress}/";
                }
                ...
                if (notify && isSystemProxyEnabled != 0 && !string.IsNullOrEmpty(systemProxyAddress))
                {
                    OnSystemProxyChangedNotify?.Invoke(systemProxyAddress);
                }
