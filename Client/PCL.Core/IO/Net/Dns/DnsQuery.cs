...
public class DnsQuery : IDisposable
{
    ...
    public async Task<DnsMessage?> QueryAsync(string host, DnsQueryType qType, CancellationToken cts = default)
    {
        ...
        {
            LogWrapper.Warn(ModuleName, $"Failed to resolve DNS for {host} using DoH: {ex.Message}, trying authoritative UDP DNS");
        }

        try
        {
            using var udpClient = new DnsUdpClient(IPAddress.Parse("223.5.5.5"));
            return await udpClient.Query(DnsQueryFactory.CreateQuery(host, qType), cts);
        }
        catch (Exception ex)
        {
            LogWrapper.Warn(ModuleName, $"Failed to resolve DNS for {host} using UDP: {ex.Message}, use system default DNS");
        }
