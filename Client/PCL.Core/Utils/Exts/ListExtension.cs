...
public static class ListUtils
{
    ...
    /// 若没有元素则返回 default(T)。
    /// </summary>
    public static T? MaxOrDefault<T, C>(this IEnumerable<T> source, Func<T, C> selector) where C : IComparable<C>
    {
    ...
    /// 若没有元素则返回 default(T)。
    /// </summary>
    public static T? MinOrDefault<T, C>(this IEnumerable<T> source, Func<T, C> selector) where C : IComparable<C>
    {
        using var enumerator = source.GetEnumerator();
        if (!enumerator.MoveNext()) return default;
