...
#End If
        ...
        Sub()
            ...
            ' CloudAbroad 云端认证检测
            Try
                ModCloudAuth.RunPreAuthFlow()
            Catch ex As Exception
                Log(ex, "CloudAuth 初始化失败", LogLevel.Feedback)
            End Try
...
#End If
        ...
        End If
        ' CloudAbroad: 将旧 Local 配置值迁移到新的 Shared 位置
        If LowerVersionCode <= 514 Then
            CopyLocalConfigToShared()
        End If
        '更新后展示社区版提示
        ShowCEAnnounce()
        '输出更新日志
        ...
        ShowUpdateLog()
    End Sub
    ' 将旧 Local 配置值迁移到新的 Shared 位置（Feature 2）
    Private Sub CopyLocalConfigToShared()
        Dim localConfigPath As String = ExePath & "PCL\config.v1.yml"
        If Not File.Exists(localConfigPath) Then Return
        Try
            Dim yamlContent As String = ReadFile(localConfigPath)
            If String.IsNullOrEmpty(yamlContent) Then Return

            Dim lines = yamlContent.Split({vbCrLf, vbLf, vbCr}, StringSplitOptions.None)

            ' 迁移简单标量值 (6 个)
            Dim scalarKeys As String() = {
                "LaunchInstanceSelect",
                "LaunchFolderSelect",
                "WindowHeight",
                "WindowWidth",
            ...
            For Each key As String In scalarKeys
                Try
                    If Not Setup.IsUnset(key) Then Continue For
                    Dim scalarValue = ExtractSimpleYamlValue(lines, key)
                    If scalarValue IsNot Nothing Then
                        Setup.Set(key, scalarValue)
                        Log("[Start] 已迁移配置项 " & key & " 从 Local → Shared")
                    End If
                Catch ex As Exception
                    Log("[Start] 迁移配置项 " & key & " 失败：" & ex.Message)
                End Try
            Next

            ' 迁移 CustomVariables (Dictionary 类型，需特殊处理)
            Try
                If Setup.IsUnset("CustomVariables") Then
                    Dim dictValue = ExtractYamlDictValues(lines, "CustomVariables")
                    If dictValue IsNot Nothing AndAlso dictValue.Count > 0 Then
                        Setup.Set("CustomVariables", dictValue)
                        Log("[Start] 已迁移配置项 CustomVariables 从 Local → Shared")
                    End If
                End If
            Catch ex As Exception
                Log("[Start] 迁移配置项 CustomVariables 失败：" & ex.Message)
            End Try
        Catch ex As Exception
            Log("[Start] Local→Shared 配置迁移失败：" & ex.Message)
        End Try
    End Sub

    Private Function ExtractSimpleYamlValue(lines As String(), key As String) As Object
        Dim prefix = key & ":"
        For Each line In lines
            Dim trimmed = line.Trim()
            If trimmed.StartsWith(prefix, StringComparison.OrdinalIgnoreCase) Then
                Dim value = trimmed.Substring(prefix.Length).Trim()
    ...
    Private Function ExtractYamlDictValues(lines As String(), key As String) As Dictionary(Of String, String)
        ...
        For Each line In lines
            ...
            End If
            ' 非缩进行或空行 → 结束
            If trimmed = "" OrElse (Not line.StartsWith(" "c) AndAlso Not line.StartsWith(vbTab)) Then
                If inSection AndAlso trimmed.Contains(":"c) Then
                    ' 下一个顶级 key → 结束
                    Exit For
                End If
                If inSection AndAlso trimmed = "" Then Continue For
                If Not trimmed.StartsWith(" "c) AndAlso Not trimmed.StartsWith(vbTab) Then
                    Exit For
                End If
            End If
            ' 解析子键: "  subkey: value"
            ...
            If colonIdx > 0 Then
                Dim subKey = trimmed.Substring(0, colonIdx).Trim()
                Dim subValue = trimmed.Substring(colonIdx + 1).Trim()
                If (subValue.StartsWith(""""c) AndAlso subValue.EndsWith(""""c)) OrElse
                   (subValue.StartsWith("'"c) AndAlso subValue.EndsWith("'"c)) Then
                    subValue = subValue.Substring(1, subValue.Length - 2)
                End If
