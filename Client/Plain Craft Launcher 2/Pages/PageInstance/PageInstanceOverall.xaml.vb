...
#Region "卡片：管理"
    ...
    Private Sub BtnManageMinePannel_Click(sender As Object, e As EventArgs) Handles BtnManageMinePannel.Click
        ...
        If isSynced Then
            ' 取消自动同步
            If MyMsgBox("确定要取消此实例与云端的自动同步吗？" & vbCrLf &
                       "取消后该实例将不再从云端获取增量更新。", "取消云端同步", "确认取消", "保持同步", IsWarn:=True) = 1 Then
                Try
                    Dim pclDir = System.IO.Path.Combine(inst.PathInstance, "PCL")
                    Dim infoPath = System.IO.Path.Combine(pclDir, "cloud_info.json")
                    Dim iniPath = System.IO.Path.Combine(pclDir, "PCL.ini")
        ...
        Else
            ' 加入云同步
            If String.IsNullOrEmpty(ModCloudAuth.CloudServerUrl) Then
                Dim domain As String = MyMsgBoxInput("连接到云端服务器", "请输入你要连接的服务器域名或 IP 地址：",
                ...
                Sub()
                    ...
                    Sub()
                        ...
                        Else
                            DoMarkInstanceOverall(inst)
                        End If
                    End Sub)
                End Sub, "Cloud Connect for Toggle")
                Return
            End If
    ...
    ' 执行标记逻辑（入口 C 共用）
    Private Sub DoMarkInstanceOverall(inst As McInstance)
        RunInNewThread(
        Sub()
            Try
                Dim localPack = ModCloudInfo.GenerateLocalInfo(inst.PathInstance)
                ...
                Dim topMatches = ModCloudMatch.FindTopMatches(localPack, cloudInfoList, topN:=1)
                If topMatches IsNot Nothing AndAlso topMatches.Count > 0 AndAlso topMatches(0).Score >= ModCloudMatch.MatchThreshold Then
                    ...
                    Sub()
                        Try
                            ...
                            If MyMsgBox(msg, "发现可同步的云端实例", "标记", "取消") = 1 Then
                                ...
                                RunInNewThread(
                                Sub()
                                    Dim marked = ModCloudDiscovery.MarkAsCloudManaged(inst.PathInstance, cloudInfo)
                                    RunInUi(
                                    Sub()
                                        If marked Then
                    ...
                    Sub()
                        Try
                            ...
                            For Each ci In cloudInfoList
                                selections.Add(New MyListItem With {.Title = ci.InstanceId, .MinHeight = 36})
                            Next
                            Dim selectedIndex As Integer? = MyMsgBoxSelect(selections, "选择要绑定的云端实例", "绑定", "取消")
                            If selectedIndex.HasValue Then
                                Dim cloudInfo = cloudInfoList(selectedIndex.Value)
                                RunInNewThread(
                                Sub()
                                    Dim marked = ModCloudDiscovery.MarkAsCloudManaged(inst.PathInstance, cloudInfo)
                                    RunInUi(
                                    Sub()
                                        ...
                                        End If
                                    End Sub)
                                End Sub, "Cloud Mark Instance Toggle Manual")
                            End If
                        Finally
                            waitHandle.Set()
                        End Try
