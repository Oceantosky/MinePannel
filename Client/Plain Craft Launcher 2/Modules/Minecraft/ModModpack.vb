        ...
        Try
            ...
            Select Case PackType
                ...
                Case 1
                    ...
                    Return InstallPackHMCL(File, Archive, ArchiveBaseFolder, InstanceName, isOnlineInstall)
                Case 2
                    Log("[ModPack] 整合包种类：MMC")
                    Return InstallPackMMC(File, Archive, ArchiveBaseFolder, InstanceName, isOnlineInstall)
                Case 3
                    Log("[ModPack] 整合包种类：MCBBS")
                    Return InstallPackMCBBS(File, Archive, ArchiveBaseFolder, InstanceName, isOnlineInstall)
                Case 4
                ...
                Case 9
                    ...
                    Return InstallPackLauncherPack(File, Archive, ArchiveBaseFolder, InstanceName, isOnlineInstall)
                Case Else
                    Log("[ModPack] 整合包种类：未能识别，假定为压缩包")
                    Return InstallPackCompress(File, Archive, InstanceName, isOnlineInstall)
            End Select
        Finally
...
#Region "CurseForge"
    Private Function InstallPackCurseForge(FileAddress As String, Archive As Compression.ZipArchive, ArchiveBaseFolder As String,
        ...
        Dim MergeLoaders As List(Of LoaderBase) = McInstallLoader(Request, IgnoreDump:=isOnlineInstall)
        '构造总加载器
...
#Region "HMCL"
    Private Function InstallPackHMCL(FileAddress As String, Archive As Compression.ZipArchive, ArchiveBaseFolder As String, Optional InstanceName As String = Nothing, Optional isOnlineInstall As Boolean = False) As LoaderCombo(Of String)
        '读取 Json 文件
        Dim Json As JObject
        Try
...
#Region "MultiMC"
    ...
    End Class
    Private Function InstallPackMMC(FileAddress As String, Archive As Compression.ZipArchive, ArchiveBaseFolder As String, Optional InstanceName As String = Nothing, Optional isOnlineInstall As Boolean = False) As LoaderCombo(Of String)
        '读取 Json 文件
...
#End Region
        ...
        '获取实例名
        If InstanceName Is Nothing Then
            InstanceName = If(RegexSeek(PackInstance, "(?<=\nname\=)[^\n]+"), "")
            Dim Validate As New ValidateFolderName(McFolderSelected & "versions")
            If Validate.Validate(InstanceName) <> "" Then InstanceName = ""
            If InstanceName = "" Then InstanceName = MyMsgBoxInput("输入实例名称", "", "", New ObjectModel.Collection(Of Validate) From {Validate})
            If String.IsNullOrEmpty(InstanceName) Then Throw New CancelledException
...
#Region "MCBBS"
    Private Function InstallPackMCBBS(FileAddress As String, Archive As Compression.ZipArchive, ArchiveBaseFolder As String,
                                      Optional InstanceName As String = Nothing, Optional isOnlineInstall As Boolean = False) As LoaderCombo(Of String)
        '读取 Json 文件
        Dim Json As JObject
        Try
...
#Region "带启动器的压缩包"
    Private Function InstallPackLauncherPack(FileAddress As String, Archive As Compression.ZipArchive, ArchiveBaseFolder As String, Optional InstanceName As String = Nothing, Optional isOnlineInstall As Boolean = False) As LoaderCombo(Of String)
        '获取解压路径
        ...
        Dim Loader As New LoaderCombo(Of String)("解压压缩包", {
            ...
            Sub(Task As LoaderTask(Of String, Integer))
                ...
                OpenExplorer(TargetFolder)
                '加入文件夹列表
                If InstanceName Is Nothing Then InstanceName = GetFolderNameFromPath(TargetFolder)
...
#Region "普通压缩包"
    Private Function InstallPackCompress(FileAddress As String, Archive As Compression.ZipArchive, Optional InstanceName As String = Nothing, Optional isOnlineInstall As Boolean = False) As LoaderCombo(Of String)
        ...
        If InstanceName Is Nothing Then InstanceName = Match.Groups(1).Value
