    ...
    Private Sub McLaunchStart(Loader As LoaderTask(Of McLaunchOptions, Object))
        ...
        Try
            ...
            Dim Loaders As New List(Of LoaderBase) From {
                New LoaderTask(Of Integer, Integer)("云端状态对齐", AddressOf McLaunchCloudHook) With {.ProgressWeight = 5},
...
#End If
        ...
        End If
    End Sub
    Private Sub McLaunchCloudHook(Loader As LoaderTask(Of Integer, Integer))
        ...
        If cloudInfo IsNot Nothing Then
            McLaunchLog("检测到云端实例标识，开始检查云端状态...")
            Try
                ...
                Else
                    McLaunchLog("云端状态无需更新或同步跳过。")
                End If
            Catch ex As Exception
                McLaunchLog("云端状态对齐发生错误：" & ex.Message)
            End Try
        Else
            McLaunchLog("当前实例非云端托管，正常启动...")
        End If
    End Sub
...
#Region "启动参数"
    ...
    Private Sub McLaunchArgumentMain(Loader As LoaderTask(Of String, List(Of McLibToken)))
        ...
        ' --- CloudAbroad 动态联机通道拦截 ---
        If String.IsNullOrWhiteSpace(Server) Then
            Dim linkJsonPath As String = McInstanceSelected.PathIndie & "link.json"
            If File.Exists(linkJsonPath) Then
                Try
                    Dim linkJson As String = ReadFile(linkJsonPath)
                    Dim linkObj As Newtonsoft.Json.Linq.JObject = Newtonsoft.Json.Linq.JObject.Parse(linkJson)
                    If linkObj("server") IsNot Nothing Then
                        Server = linkObj("server").ToString()
                        If linkObj("port") IsNot Nothing Then
                            Server &= ":" & linkObj("port").ToString()
                        End If
